...
import { singlePatientData } from "store/feature/patientSlice";
const SinglePatient = () => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const { singleData, status } = useSelector((state) => state.bloodDonor);
  useEffect(() => {
    dispatch(singlePatientData(id));
  }, [dispatch, id]);
                ...
                {singleData && (
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                      ...
                      <td className="whitespace-nowrap px-6 py-3 text-sm text-gray-900 dark:text-gray-100"></td>
                    </tr>
                    <tr>
                      <th className="text-black px-6 py-3 text-left text-xs font-bold tracking-wider dark:text-gray-300">
                        LastDonted Date
                      ...
                      <td className="whitespace-nowrap px-6 py-3 text-sm text-gray-900 dark:text-gray-100">
                        {/* {new Date(singleData.lastDonated).toLocaleString()} */}
                      </td>
                    </tr>
