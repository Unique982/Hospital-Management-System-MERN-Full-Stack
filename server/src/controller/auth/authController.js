...
exports.registerUser = async (req, res, next) => {
  ...
  console.log(req.body, "body kunkun data aayo ");
  // check email user already exist or not
  const userExist = await users.findOne({ where: { email } });
  if (userExist) {
    return res
      .status(400)
      .json({ message: "Email is already registered.", data: [] });
  }
  ...
  console.log(newUser);
  console.log(newUser.id, "id pass vaya ko xa ki nai");
  // patient table insert
  const patinet = await patients.create({
    user_id: newUser.id,
    firstName,
    lastName,
    address,
  ...
  console.log(patinet, "patient ,ma k xa");
  await sendEmail({
    email: email,
...
Hospital Management System Team`,
  ...
  res.status(200).json({ message: "Added New Patient Succssfully" });
};
...
exports.loginUser = async (req, res, next) => {
  try {
    ...
    if (isMatched) {
      ...
      return res.json({ message: "User Logged successfully!", token: token });
    } else {
